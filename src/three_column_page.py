    ...
    for fn in sys.argv[1:]:
        ...
        for i in range(df['col'].max() + 1):
            for j in range(df['line'].max() + 1):
                ...
                test=copy.copy(l)
                #if not box.empty:
                 #       print("'"+lastline+"'"+"\t\t({},{}), ({},{})".format(int(box.iloc[0]['xmin']), int(box.iloc[0]['ymin']), int(box.iloc[0]['xmax']), int(box.iloc[0]['ymax'])))
                  #  else:
                   #     print(lastline)
                ...
                #result = filter(None, [x.strip() for x in re.findall(r"\b[A-Z\s]+\b", test)])
                if test.split(' ', 1)[0].isupper():
                    if "Incorporated" in test or "Incor" in test or "Inco" in test or "Incorp" in test or "Incorpo" in test or "Incorpor" in test or "Incorpora" in test or "Incorporat" in test or "Incorporate" in test or "In-" in test or "Incorpor-" in test or "Incorpo-" in test:
                        result = filter(None, [x.strip() for x in re.findall(r"\b[A-Z\s]+\b", test)])
                        ...
                        box = df.loc[(df['col'] == i)
                                           ...
                                           & (df['type'] == elemType.line)]
                        if not box.empty:
                            print("'"+element+"'"+"\t\t({},{}), ({},{})".format(int(box.iloc[0]['xmin']), int(box.iloc[0]['ymin']), int(box.iloc[0]['xmax']), int(box.iloc[0]['ymax'])))
                        else:
                    ...
                    if "In-" in test:
                        ...
                        for element in result:
                            pass
                        box = df.loc[(df['col'] == i) & (df['line'] == j) & (df['type'] == elemType.line)]
                        if not box.empty:
                            print("'"+element+"'"+"\t\t({},{}), ({},{})".format(int(box.iloc[0]['xmin']), int(box.iloc[0]['ymin']), int(box.iloc[0]['xmax']), int(box.iloc[0]['ymax'])))
                        else:
                            print("MISSED COMPANY NAME")
                                #Other words
                    if "Manufactures" in test or "Established" in test:
                        result = filter(None, [x.strip() for x in re.findall(r"\b[A-Z\s]+\b", test)])
                        for element in result:
                            pass
                        box = df.loc[(df['col'] == i) & (df['line'] == j) & (df['type'] == elemType.line)]
